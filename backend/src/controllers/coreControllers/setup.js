...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      erp_crm_app_email: email,
      erp_crm_app_company_email: email,
      erp_crm_app_timezone: timezone,
      erp_crm_app_country: country,
      erp_crm_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully Nextus ERP CRM App Setup',
  });
};
